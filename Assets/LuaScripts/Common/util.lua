...
local Logger = require("Logger")("util")
...
local function print_func_ref_by_csharp()
    ...
    for k, v in pairs(registry) do
        if type(k) == 'number' and type(v) == 'function' and registry[v] == k then
            ...
            Logger:Error("lua往C#中的注入的方法没有被置为null print_func_ref_by_csharp form {0} , line = {1}, details:{2}", info.short_src, info.linedefined, Logger.Dump(info))
        end
    end
end
